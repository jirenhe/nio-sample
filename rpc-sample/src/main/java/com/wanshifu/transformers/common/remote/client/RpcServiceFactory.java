...
public class RpcServiceFactory {
    ...
    public static <T> T getKeepAliveRemoteService(Class<T> serviceClass, String address, int port) throws Exception {
        return getKeepAliveRemoteService(serviceClass, address, port, new ProtostuffSerializer());
    }

    @SuppressWarnings("unchecked")
    public static <T> T getKeepAliveRemoteService(Class<T> serviceClass, String address, int port, Serializer serializer) throws Exception {
        ...
        if (rpcClient == null || !rpcClient.isAlive()) {
            synchronized (RpcServiceFactory.class) {
                ...
                if (rpcClient == null || !rpcClient.isAlive()) {
                    rpcClient = new NettyRpcClient(address, port, serializer, 5, 5);
                    rpcClient.connect();
    ...
    public static <T> T getShortConnectRemoteService(Class<T> serviceClass, String address, int port) throws Exception {
        return getShortConnectRemoteService(serviceClass, address, port, new ProtostuffSerializer());
    }

    @SuppressWarnings("unchecked")
    public static <T> T getShortConnectRemoteService(Class<T> serviceClass, String address, int port, Serializer serializer) throws Exception {
        if (!serviceClass.isInterface()) {
            throw new RpcException(String.format("this class : %s is not a interface", serviceClass.getName()));
        }
        return (T) Proxy.newProxyInstance(RpcServiceFactory.class.getClassLoader(), new Class[]{serviceClass},
                new ShortRpcServiceProxy(serviceClass, new NettyRpcClient(address, port, serializer)));
    }
}
